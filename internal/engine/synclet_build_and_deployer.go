...
func (sbd *SyncletBuildAndDeployer) BuildAndDeploy(ctx context.Context, specs []model.TargetSpec, stateSet store.BuildStateSet) (store.BuildResultSet, error) {
	...
	if len(resources) == 0 || len(images) != 1 {
		return store.BuildResultSet{}, RedirectToNextBuilderf("Synclet-deploy limited to a single k8s image")
	}
	...
	if err := sbd.canSyncletBuild(ctx, image, state); err != nil {
		return store.BuildResultSet{}, WrapRedirectToNextBuilder(err)
	}

	return sbd.updateViaSynclet(ctx, image, state)
}
